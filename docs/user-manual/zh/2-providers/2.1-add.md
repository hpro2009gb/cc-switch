...
如果你已经在 Chrome 中登录 ChatGPT，Codex OAuth 区块还提供「从 Chrome 导入」按钮：

1. 点击「从 Chrome 导入」。
2. CC Switch 会在本机 Chrome 的 `Default` 和 `Profile *` 配置中查找 ChatGPT 会话。
3. Rust 后端使用本地会话调用 `chatgpt.com/api/auth/session`，再把返回的账号加入 Codex OAuth。

原始 cookie 不会返回到前端，也不会写入日志。导入的会话 cookie 会保存在本地认证存储中，以便 access token 过期后重新验证。请把它视为高度敏感的认证凭据：只在可信的 CC Switch 构建版本中使用；如需撤销 CC Switch 对该会话的访问，请移除账号或注销所有账号。

如果 Chrome 因浏览器安全策略或配置文件锁定等原因无法解密 cookie 数据库，请改用 Device Code 登录流程。
...
- 从 Chrome 导入的账号会使用本地保存的 ChatGPT 会话 cookie 重新验证 access token
- **不支持**导出 Token（防止泄露）
...
| Chrome 导入失败      | 未找到有效 Chrome 会话          | 先在 Chrome 登录 chatgpt.com，或改用 Device Code 登录 |
