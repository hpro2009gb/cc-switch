...
If you are already signed in to ChatGPT in Chrome, the Codex OAuth section also provides **Import from Chrome**:

1. Click **Import from Chrome**.
2. CC Switch checks the local Chrome `Default` and `Profile *` profiles for a ChatGPT session.
3. The Rust backend exchanges the local session with `chatgpt.com/api/auth/session` and adds the returned account to Codex OAuth.

Raw cookies are not returned to the frontend or written to logs. The imported session cookie is stored in the local auth store so the access token can be revalidated after expiry. Treat this as highly sensitive credential material: only use it with a trusted CC Switch build, and remove the account or log out all accounts to revoke it from CC Switch.

If Chrome cannot decrypt the profile database (for example, because of a browser security or profile-locking change), use the Device Code login flow instead.
...
- Accounts imported from Chrome revalidate their access token through the locally stored ChatGPT session cookie
...
| Chrome import failed        | No active Chrome session found    | Sign in to chatgpt.com in Chrome or use Device Code login |
